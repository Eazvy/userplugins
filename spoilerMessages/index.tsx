...
const SpoilerToggle: ChatBarButtonFactory = ({ isMainChat }) => {
    ...
    return (
        <ChatBarButton
            ...
            onClick={toggle}
        >
            <svg width="26" height="26" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style={{ scale: "1", marginTop:"-5" }}>
...
export function spoiler(upload: Upload) {
    const file = upload.filename;
    if (typeof file !== "string") {
        console.error("Invalid filename:", file);
        return "SPOILER_undefined";
    }
    ...
    return settings.store.isEnabled ? `SPOILER_${fileName}${ext}` : file;
}
...
export default definePlugin({
    ...
    authors: [EquicordDevs.omaw, EquicordDevs.KrystalSkull],
   ...
   patches: [
    {
        ...
        replacement: [
            {
            match: /async uploadFiles\((\w+)\)\s*{/,
            replace: "async uploadFiles($1){ $1.forEach(f => f.filename = $self.spoiler(f));"
            }
