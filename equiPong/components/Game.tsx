...
export function Game({ onClose, startEmoji }: GameProps) {
    ...
    useEffect(() => {
        ...
        const dprValue = typeof globalThis.devicePixelRatio === "number" ? globalThis.devicePixelRatio : 1;
        const dpr = dprValue > 0 ? dprValue : 1;
    ...
    const handleGameKeyCapture = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const renderDuelTop = () => {
        ...
        const { viewerScore } = duel;
        const { opponentScore } = duel;
